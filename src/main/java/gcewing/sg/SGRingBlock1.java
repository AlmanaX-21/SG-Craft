...
public class SGRingBlock1 extends SGRingBlock0 {
   static String[] textures = new String[]{"stargateBlock1", "stargateRing1", "stargateChevron1"};
   static BaseMod.ModelSpec[] models = new BaseMod.ModelSpec[]{new BaseMod.ModelSpec("block/sg_ring_block.smeg", new String[]{"stargateBlock1", "stargateRing1"}), new BaseMod.ModelSpec("block/sg_ring_block.smeg", new String[]{"stargateBlock1", "stargateChevron1"})};

   protected String getRendererClassName() {
      return "SGRingBlock1Renderer";
   }
   ...
   public boolean onBlockActivated(World world, BlockPos pos, IBlockState state, EntityPlayer player, EnumFacing side, float cx, float cy, float cz) {
      ...
      if (te.isMerged) {
         ...
         if (block instanceof SGBaseBlock1) {
            ((SGBaseBlock1)block).onBlockActivated(world, te.basePos, baseState, player, side, cx, cy, cz);
         }
      ...
      for(int j = -6; j <= 0; ++j) {
         ...
         for(int i = -reach; i <= reach; ++i) {
            for(int k = -reach; k <= reach; ++k) {
               ...
               if (block instanceof SGBaseBlock1) {
                  SGBaseBlock1 base = (SGBaseBlock1)block;
                  if (!te.isMerged) {
