...
public class SGBaseBlock4 extends SGBaseBlock0 {
   protected static String[] textures = new String[]{"stargateBlock4", "stargateRing4", "stargateBase_front4"};
         ...
         for(int i = -2; i <= 2; ++i) {
            for(int j = 0; j <= 4; ++j) {
               if (i != 0 || j != 0) {
                  ...
                  if (block instanceof SGRingBlock4) {
                     ((SGRingBlock4)block).mergeWith(world, rp, pos);
                  }
               }
            }
         }

         te.checkForLink((byte)4);
      }
   ...
   int getRingBlockType(World world, BlockPos pos) {
      ...
      if (block == Blocks.air) {
         ...
         if (block == SGCraft.sgRingBlock4 && !SGCraft.sgRingBlock4.isMerged(world, pos)) {
            IBlockState state = BaseBlockUtils.getWorldBlockState(world, pos);
            switch ((Integer)state.getValue(SGRingBlock4.VARIANT)) {
               case 0:
                  return 1;
               case 1:
                  return 2;
   ...
   void unmergeRingBlock(World world, BlockPos pos, BlockPos ringPos) {
      ...
      if (block instanceof SGRingBlock4) {
         ((SGRingBlock4)block).unmergeFrom(world, ringPos, pos);
      }
