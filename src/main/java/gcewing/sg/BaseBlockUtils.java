...
public class BaseBlockUtils {
   public static String getNameForBlock(Block block) {
      return block != null ? Block.blockRegistry.getNameForObject(block).toString() : "";
   }
      ...
      if (block.isProvidingWeakPower(world, pos.x, pos.y, pos.z, side.ordinal()) > 0) {
         ...
         if (block.shouldCheckWeakPower(world, pos.x, pos.y, pos.z, side.ordinal())) {
            for(EnumFacing side2 : BaseUtils.facings) {
               if (side2 != BaseUtils.oppositeFacing(side)) {
   ...
   public static IBlockState getBlockStateFromItemStack(ItemStack stack) {
      ...
      return (IBlockState)(block instanceof BaseBlock ? ((BaseBlock)block).getStateFromMeta(meta) : new MetaBlockState(block, meta));
   }
   ...
   public static int getMetaFromBlockState(IBlockState state) {
      return state instanceof MetaBlockState ? ((MetaBlockState)state).meta : ((BaseBlock)state.getBlock()).getMetaFromState(state);
   }
   ...
   public static IBlockState getDefaultBlockState(Block block) {
      return (IBlockState)(block instanceof BaseBlock ? ((BaseBlock)block).getDefaultState() : new MetaBlockState(block, 0));
   }
