...
public class SGRingBlock3 extends SGRingBlock0 {
   static String[] textures = new String[]{"stargateBlock3", "stargateRing3", "stargateChevron3"};
   static BaseMod.ModelSpec[] models = new BaseMod.ModelSpec[]{new BaseMod.ModelSpec("block/sg_ring_block.smeg", new String[]{"stargateBlock3", "stargateRing3"}), new BaseMod.ModelSpec("block/sg_ring_block.smeg", new String[]{"stargateBlock3", "stargateChevron3"})};

   protected String getRendererClassName() {
      return "SGRingBlock3Renderer";
   }
   ...
   public boolean onBlockActivated(World world, BlockPos pos, IBlockState state, EntityPlayer player, EnumFacing side, float cx, float cy, float cz) {
      ...
      if (te.isMerged) {
         ...
         if (block instanceof SGBaseBlock3) {
            ((SGBaseBlock3)block).onBlockActivated(world, te.basePos, baseState, player, side, cx, cy, cz);
         }
      ...
      for(int j = -6; j <= 0; ++j) {
         ...
         for(int i = -reach; i <= reach; ++i) {
            for(int k = -reach; k <= reach; ++k) {
               ...
               if (block instanceof SGBaseBlock3) {
                  SGBaseBlock3 base = (SGBaseBlock3)block;
                  if (!te.isMerged) {
