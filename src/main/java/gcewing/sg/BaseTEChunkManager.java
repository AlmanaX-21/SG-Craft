...
public class BaseTEChunkManager implements ForgeChunkManager.LoadingCallback {
   ...
   protected ForgeChunkManager.Ticket newTicket(World world) {
      return ForgeChunkManager.requestTicket(this.base, world, Type.NORMAL);
   }

   public void ticketsLoaded(List<ForgeChunkManager.Ticket> tickets, World world) {
      for(ForgeChunkManager.Ticket ticket : tickets) {
         NBTTagCompound nbt = ticket.getModData();
         if (nbt != null && nbt.getString("type").equals("TileEntity")) {
            int x = nbt.getInteger("xCoord");
            int y = nbt.getInteger("yCoord");
            int z = nbt.getInteger("zCoord");
            TileEntity te = world.getTileEntity(x, y, z);
            if (!(te instanceof BaseTileEntity) || !this.reinstateChunkTicket((BaseTileEntity)te, ticket)) {
   ...
   protected ForgeChunkManager.Ticket getChunkTicket(BaseTileEntity te) {
      if (te.chunkTicket == null) {
         te.chunkTicket = this.newTicket(te.getWorldObj());
      }
