...
public class SGRingBlock4 extends SGRingBlock0 {
   static String[] textures = new String[]{"stargateBlock4", "stargateRing4", "stargateChevron4"};
   static BaseMod.ModelSpec[] models = new BaseMod.ModelSpec[]{new BaseMod.ModelSpec("block/sg_ring_block.smeg", new String[]{"stargateBlock4", "stargateRing4"}), new BaseMod.ModelSpec("block/sg_ring_block.smeg", new String[]{"stargateBlock4", "stargateChevron4"})};

   protected String getRendererClassName() {
      return "SGRingBlock4Renderer";
   }
   ...
   public boolean onBlockActivated(World world, BlockPos pos, IBlockState state, EntityPlayer player, EnumFacing side, float cx, float cy, float cz) {
      ...
      if (te.isMerged) {
         ...
         if (block instanceof SGBaseBlock4) {
            ((SGBaseBlock4)block).onBlockActivated(world, te.basePos, baseState, player, side, cx, cy, cz);
         }
      ...
      for(int j = -6; j <= 0; ++j) {
         ...
         for(int i = -reach; i <= reach; ++i) {
            for(int k = -reach; k <= reach; ++k) {
               ...
               if (block instanceof SGBaseBlock4) {
                  SGBaseBlock4 base = (SGBaseBlock4)block;
                  if (!te.isMerged) {
