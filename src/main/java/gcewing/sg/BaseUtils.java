...
public class BaseUtils {
   ...
   public static int packedColor(double red, double green, double blue) {
      return (int)(red * (double)255.0F) << 16 | (int)(green * (double)255.0F) << 8 | (int)(blue * (double)255.0F);
   }
   ...
   public static AxisAlignedBB boxIntersection(AxisAlignedBB box1, AxisAlignedBB box2) {
      ...
      return minX < maxX && minY < maxY && minZ < maxZ ? newAxisAlignedBB(minX, minY, minZ, maxX, maxY, maxZ) : null;
   }
   ...
   public static <T extends WorldSavedData> T getWorldData(World world, Class<T> cls, String name) {
      ...
      if (result == null) {
         try {
            result = (T)(cls.getConstructor(String.class).newInstance(name));
   ...
   static {
      horizontalFacings = new EnumFacing[]{EnumFacing.SOUTH, EnumFacing.WEST, EnumFacing.NORTH, EnumFacing.EAST};
   }
}
