...
public class SGBaseBlock3 extends SGBaseBlock0 {
   protected static String[] textures = new String[]{"stargateBlock3", "stargateRing3", "stargateBase_front3"};
         ...
         for(int i = -2; i <= 2; ++i) {
            for(int j = 0; j <= 4; ++j) {
               if (i != 0 || j != 0) {
                  ...
                  if (block instanceof SGRingBlock3) {
                     ((SGRingBlock3)block).mergeWith(world, rp, pos);
                  }
               }
            }
         }
   ...
   int getRingBlockType(World world, BlockPos pos) {
      ...
      if (block == Blocks.air) {
         ...
         if (block == SGCraft.sgRingBlock3 && !SGCraft.sgRingBlock3.isMerged(world, pos)) {
            IBlockState state = BaseBlockUtils.getWorldBlockState(world, pos);
            switch ((Integer)state.getValue(SGRingBlock3.VARIANT)) {
               case 0:
                  return 1;
               case 1:
                  return 2;
   ...
   void unmergeRingBlock(World world, BlockPos pos, BlockPos ringPos) {
      ...
      if (block instanceof SGRingBlock3) {
         ((SGRingBlock3)block).unmergeFrom(world, ringPos, pos);
      }
