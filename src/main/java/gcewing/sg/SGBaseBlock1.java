...
public class SGBaseBlock1 extends SGBaseBlock0 {
   protected static String[] textures = new String[]{"stargateBlock1", "stargateRing1", "stargateBase_front1"};
   ...
   int getRingBlockType(World world, BlockPos pos) {
      ...
      if (block == Blocks.air) {
         ...
         if (block == SGCraft.sgRingBlock1 && !SGCraft.sgRingBlock1.isMerged(world, pos)) {
            IBlockState state = BaseBlockUtils.getWorldBlockState(world, pos);
            switch ((Integer)state.getValue(SGRingBlock1.VARIANT)) {
               case 0:
                  return 1;
               case 1:
                  return 2;
   ...
   void unmergeRingBlock(World world, BlockPos pos, BlockPos ringPos) {
      ...
      if (block instanceof SGRingBlock1) {
         ((SGRingBlock1)block).unmergeFrom(world, ringPos, pos);
      }
